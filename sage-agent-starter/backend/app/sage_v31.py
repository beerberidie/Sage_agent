...
class SageV31Client:
    ...
    def _get_connection(self, session) -> Connection:
        conn = session.exec(
            select(Connection).where(
                Connection.tenant_id == self.tenant_id, Connection.provider == "sbca"
            )
        ).first()
        if not conn or not conn.access_token:
            raise HTTPException(
                status_code=400, detail="Sage connection not configured."
            )
        return conn
    ...
    async def list_sales_invoices(
        self, updated_since: Optional[str] = None, items_per_page: int = 50
    ):
    ...
    async def list_contacts(
        self, search: Optional[str] = None, items_per_page: int = 50
    ):
        params = {"items_per_page": items_per_page}
        if search:
